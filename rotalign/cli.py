...
def get_args():
    parser = argparse.ArgumentParser(
        description="Rotational alignment"
    )
    parser.add_argument(
        "input_tifs",
        type=str,
...
def run(mov_in_fn, coord_fn):
    ...
    COORDS_TAB = COORDS_TAB[['Frame #', 'Time (seconds post fertilisation', 'name', 'Xcm', 'Ycm','Zcm', 'Xpb', 'Ypb', 'Zpb', 'Xsa', 'Ysa', 'Zsa']]
    COORDS_TAB["time"] = COORDS_TAB["Time (seconds post fertilisation"]
    ...
    coords_tab = coords_tab.groupby("frame")[['time', 'Xcm', 'Ycm','Zcm', 'Xpb', 'Ypb', 'Zpb', 'Xsa', 'Ysa', 'Zsa']].mean()
    ...
    for t in trange(t_size, desc="  - centering to CM"):
        for c in range(c_size):
            ...
            shift(mov_slice, shift=[z_sft, y_sft, x_sft], order=0, output=mov_centered[t,:,c,...])
    ...
    for t in trange(t_size, desc="  - Aligning frames...", position=1):
        ...
        mg = np.mgrid[-z_iso_size//2:z_iso_size//2, -y_size//2:y_size//2, -x_size//2:x_size//2]
    ...
    print("*"*80)
    tifffile.imsave(f"{tif_path}/{tif_base_fn}_aligned.tif", mov_out, imagej=True, resolution=(1./x_pxs, 1./y_pxs), metadata={'spacing': x_pxs, 'unit': 'um', 'axes': 'TZCYX'})
    print()
